...
public class PaymentPerMonth {
	...
	public PaymentPerMonth(final LoanRequest request) {
		...
		this.interest = roundOff(calculateInterest(BigDecimal.valueOf(request
				.getNominalRate())));
		this.principal = roundOff(calculatePrincipal(this.borrowerPaymentAmount));
		this.remainingOutstandingPrincipal = roundOff(this.initialOutstandingPrincipal
				.subtract(this.principal));
	}

	public PaymentPerMonth(final LoanRequest request,
			final PaymentPerMonth previousPaymentPerMonth, final int counter) {
		this.initialOutstandingPrincipal = roundOff(previousPaymentPerMonth
				.getRemainingOutstandingPrincipal());
		this.date = generatePaymentDate(request.getStartDate(), counter);
		this.interest = roundOff(calculateInterest(BigDecimal.valueOf(request
				.getNominalRate())));
		this.principal = roundOff(calculatePrincipal(previousPaymentPerMonth
	...
	private BigDecimal roundOff(final double doubleValue) {
		...
		return bigDecimalValue.setScale(Constants.DECIMALS,
	...
	private double calculateBorrowerPaymentAmount(final LoanRequest request) {
		return borrowerPaymentAmountNumerator(request)
				/ borrowerPaymentAmountDenominator(request);
	}

	private double borrowerPaymentAmountNumerator(final LoanRequest request) {
		return request.getLoanAmount()
				* interestRateOverPaymentsPerYear(request.getNominalRate());
	}

	private double borrowerPaymentAmountDenominator(final LoanRequest request) {
		final double base = 1 + interestRateOverPaymentsPerYear(request
				.getNominalRate());
	...
	private BigDecimal calculateInterest(final BigDecimal nominalRate) {
		final BigDecimal interestInCents = (nominalRate
				.multiply(Constants.DAYS_PER_MONTH)
				.multiply(this.initialOutstandingPrincipal)).divide(
				Constants.DAYS_PER_YEAR, BigDecimal.ROUND_HALF_UP);
